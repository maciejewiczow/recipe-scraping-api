...
from shared.models.chat.chat_models import Ingredient, Ingredients, schema_name
...
@dump_response
@validate_environment(model=Environment)
def handler(event: ParseIngredientInput, context: LambdaContext, *, env: Environment):
    ...
    Ingredient.apply_schema_lang(lang)
        ...
        response = client.responses.create(
            ...
            text={
                "format": {
                    ...
                    "strict": False,
                    "schema": Ingredients.json_schema(),
                },
                "verbosity": "low",
            },
        ...
        responsesTable.put_item(
            Item=StoredResponse(
                ...
                OriginalIngredientInput=IngredientToProcessDTO(
                    content=event.ingredient.content,
                    ingredientId=event.ingredient.ingredientId,
                    recipeId=event.ingredient.recipeId,
                ),
                RetryCount=event.error.retryCount if event.error is not None else 0,
            ).model_dump(),
            ReturnValues="NONE",
        )
    except ClientError:
